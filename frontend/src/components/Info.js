...
function Info({ items, badge }) {
  //const state = location.state(); 추억 개수 전달
  const { name, image, description, isPublic, likeCount, createdAt } = items;
  const postCount = 0;
  let badgeName = [];
  if (badge.length !== 0) {
    ...
    <>
      <div style={style}>
        <div className="infoImage">
          <img
            style={{ width: "262px", height: "273px", objectFit: "cover" }}
            src={imageUrl}
            alt="그룹이미지"
          />
        </div>
        <div style={{ flexGrow: "1", margin: "10px" }}>
          <div className="groupInfo">
            ...
            <div>
              ...
              {changeModal === true ? (
                <InfoChangeModal items={items} setModal={setChangeModal} />
              ) : null}
              <button
                className="infoButton"
                style={{ color: "gray" }}
              ...
              {deleteModal === true ? (
                <InfoDeleteModal setModal={setDeleteModal} id={items.id} />
              ) : null}
            </div>
          </div>
          ...
          <div className="infoBody">{description}</div>
          <div className="badge">
