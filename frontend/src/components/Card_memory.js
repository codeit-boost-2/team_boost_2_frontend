...
function CardMemoryItems({ item }) {
  ...
  const isPublicPath =
    isPublic === true ? `/MemoryPage/${item.id}` : `/AutPage/memory/${item.id}`;
      ...
      >
        <div className="card">
          <div className="cardImg">
            {isPublic === true && image && (
              <img className="groupIMG" src={imageUrl} alt="대표이미지"></img>
            )}
          </div>
            ...
            <div>
              <div style={{ display: "flex" }}>
                <img
                  ...
                  alt="공감아이콘"
                  src="../imgs/logo_16x16.svg"
                />
                {likeCount} <div stlye={{ margin: "3px" }}></div>
                <img
                  style={{ margin: "0 7px" }}
                  alt="댓글아이콘"
                  src="../imgs/icon_bubble.svg"
                />
                {commentCount}
              </div>
            </div>
